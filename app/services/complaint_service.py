...
from app.models.complaint import Complaint, ComplaintMedia, ComplaintStatus, ComplaintStatusHistory
...
TRANSITIONS = {
    ...
    ComplaintStatus.resolved: set(), ComplaintStatus.rejected: set(),
}
...
def change_status(db: Session, complaint: Complaint, new_status: ComplaintStatus, actor: User | None, note: str | None = None, internal=False):
    if new_status == complaint.status: return
    if new_status not in TRANSITIONS[complaint.status]: raise HTTPException(422, "Invalid complaint status transition")
    old = complaint.status; complaint.status = new_status
    if new_status == ComplaintStatus.resolved: complaint.resolved_at = datetime.utcnow(); complaint.resolution_note = note
    db.add(ComplaintStatusHistory(complaint_id=complaint.id, old_status=old, new_status=new_status, changed_by=actor.id if actor else None, note=note))
def submit_analysis_result(db: Session, complaint: Complaint, analysis: AnalysisResult, actor: User | None = None) -> Complaint:
    ...
    if complaint.status == ComplaintStatus.submitted: change_status(db, complaint, ComplaintStatus.processing, actor, "Analysis result received")
    if complaint.status == ComplaintStatus.processing: change_status(db, complaint, ComplaintStatus.classified, actor, "Analysis result stored")
    return complaint
def get_complaint_for_user(db: Session, complaint_id: str, user: User) -> Complaint:
    ...
    if not complaint: raise HTTPException(404, "Complaint was not found")
    if user.role == UserRole.passenger and complaint.user_id != user.id: raise HTTPException(403, "Not permitted to access this complaint")
    return complaint
